...
class Enemy:
    ...
    def take_damage(self, amount: float):
        # Applies damage reduction based on the resistance gene
    ...
    def update(self, dt: float, player, all_enemies: list, enemy_bullets: list) -> None:
        ...
        if self.chrom.archetype in (Archetype.TANK, Archetype.STRIKER):
            ...
            if math.hypot(player.x - self.x, player.y - self.y) < self.attack_range + player.radius and self.attack_cd <= 0:
            ...
            if dist > self.attack_range:
                ...
                enemy_bullets.append(EnemyBullet(self.x, self.y, player.x, player.y, self.chrom.damage, proj_speed))
                self.damage_dealt += self.chrom.damage
                self.attack_cd = self.chrom.attack_cd 

        elif self.chrom.archetype == Archetype.SUPPORT:
            weakest_ally = min([e for e in all_enemies if e.alive and e != self], key=lambda e: e.hp/e.max_hp, default=None)
            if weakest_ally and (weakest_ally.hp / weakest_ally.max_hp) < 1.0:
                self._steer_towards(weakest_ally.x, weakest_ally.y, dt)
                if math.hypot(weakest_ally.x - self.x, weakest_ally.y - self.y) <= self.aoe_range and self.attack_cd <= 0:
    ...
    def draw(self, screen: pygame.Surface, camera) -> None:
        ...
        if self.chrom.archetype == Archetype.SUPPORT and self.attack_cd <= 0:
            pygame.draw.circle(screen, (100, 255, 100), (int(sx), int(sy)), self.aoe_range, 1)
        pygame.draw.circle(screen, self.color, (int(sx), int(sy)), self.radius)
        
        hp_ratio = max(0, self.hp / self.max_hp)
        pygame.draw.rect(screen, (255, 0, 0), (sx - 15, sy - self.radius - 10, 30, 4))
        pygame.draw.rect(screen, (0, 255, 0), (sx - 15, sy - self.radius - 10, int(30 * hp_ratio), 4))